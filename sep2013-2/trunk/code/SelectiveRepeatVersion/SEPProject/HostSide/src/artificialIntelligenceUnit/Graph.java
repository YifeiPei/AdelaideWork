...
public class Graph{
	...
	protected ArrayList<Vertex> AStarSearch(Vertex start, Vertex goal)
	{
		int count = 0;
	...
	protected void euclideanHeuristic(Node node, Vertex goal){
	...
	public void initialiseGraph(){
		MyInt index = new MyInt(0);
		startRoad = (Road)map.getRobot();
		startRobot = map.getRobot().getRobotLocation();
		//System.out.println("BEGIN:\n    Create vertex 0");
	...
	protected double dotProduct(Point p1, Point p2){
	...
	protected CollisionResult testCollision(Point A, Point B, Point C, double r){
		boolean isVertical;
		double Ax = A.getX();
		double Ay = A.getY();
		double Bx = B.getX();
	...
	protected boolean isInBetween(Point a, Point b, Point c){
