...
public class Graph{
	...
	private ArrayList<String> solutionActions;
	...
	public Graph(Map m){
		...
		solutionActions = new ArrayList<String>();
	}
	...
	public Stack<Vertex> solution(Node n, Stack<Vertex> solutionSet){
		solutionSet.push(n.getState());
		if(n.isInitial())
			return solutionSet;
		else
	...
	public Stack<Vertex> AStarSearch(Vertex start, Vertex goal)
	{
		int count = 0;
		Node initial = new Node(start, start.index());
		euclideanHeuristic(initial, goal);
		Stack<Vertex> solutionSet = new Stack<Vertex>();
			...
			if(ct.collision()){
				if(!ct.twoCollisions()){
					if(isInBetween(start.getCoordinate(), closestPoint, ct.firstCollision())){
						...
						needsMarking = true;
						deadEnd = false;
						closestPoint = ct.firstCollision();
					...
					if(d1 < d2){
						if(isInBetween(start.getCoordinate(), closestPoint, ct.firstCollision())){
							...
							needsMarking = true;
							deadEnd = false;
							closestPoint = (Point)ct.firstCollision();
						...
						if(isInBetween(start.getCoordinate(), closestPoint, ct.firstCollision())){
							...
							needsMarking = true;
							deadEnd = false;
							closestPoint = (Point)ct.secondCollision();
							//System.out.println("Change!\n    Disaster Zone, Two Collisions:\n    closestPoint = "+closestPoint+"\n");
						}
					}
				}
