...
public class RobotMap implements Map {
		...
		try{
			...
			for(Road r: roads){
				if(r.isClosed()){
					...
					e3 = document.createElement("point");
					e3.setAttribute("x", String.valueOf((int)r.getClosureLocation().getX()));
					e3.setAttribute("y", String.valueOf((int)r.getClosureLocation().getY()));
					e2.appendChild(e3);
					e1.appendChild(e2);
				}
			}
	...
	public void print(){
		...
		for(Road r: roads){
			...
			if(r.isClosed()){
				System.out.println(" ("+r.getClosureLocation().getX()+", "+r.getClosureLocation().getY()+")");
			}
			else
				System.out.println();
