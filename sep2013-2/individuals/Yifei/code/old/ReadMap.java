...
public class ReadMap
{
	...
	public void LoadBoundary (String fileName)
	{
	...
	NodeList robotStatus = doc.getElementsByTagName("robot-status");
	Node robotstat = robotStatus.item(0);
	Element r = (Element) robotstat;
	NodeList robotAttribute = r.getElementsByTagName("attribute");
	NodeList robotLocation = r.getElementsByTagName("point");
	Node location = robotLocation.item(0);
	Element locationpoint = (Element) location;
	robot[0] = Integer.parseInt(locationpoint.getAttribute("x"));
	robot[1] = Integer.parseInt(locationpoint.getAttribute("y"));
	Node robotAttri = robotAttribute.item(0);
	Element robotAtt = (Element) robotAttri;
	String face = robotAttri.getTextContent();
	int facing = Integer.parseInt(face.replaceAll("[\\D]", ""));
	robot[2] = facing;
	
	System.out.println(robot[0] + " " + robot[1] + " " + robot[2]);
