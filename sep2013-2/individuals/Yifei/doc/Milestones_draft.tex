%%%%%%LIST OF DELIVERABLE MILESTONES #2
...
\item the robot has the ability to go back to the starting position in AI mode
\end {itemize}
...
including
\begin {itemize}
\item movement speed should be an accepted low speed (5cm/s?)
\item collision detection. Once collision happened, the robot should stop immediately.
\item low power performance. (how low is low power? 10\% or 5\%?) send a warning to the operator and immediately stop, waiting for the manual control.
...
\item This one is not sure. I'm asking on forum if we need to prepare the A1 sized map site by ourselves.
...
{\bf Not decided which should go to Week 9 and which should go to Week 10. please feel free to comment.}
