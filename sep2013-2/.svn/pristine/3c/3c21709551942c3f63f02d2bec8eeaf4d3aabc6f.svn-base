...
public class MapDraw extends JPanel{
	...
	public MapDraw(Map m, Graph g){
		...
		this.setBounds(247, 13, 950, 750); //size of the map 1300x750
	...
	{
		final int radius = 4; // the radius for intersection, road closure, and obstacle
		//System.out.println(SwingUtilities.isEventDispatchThread());
		super.paintComponent(g);
		Graphics2D g2d = (Graphics2D) g;
		double calculate_scale_factor = Math.min( ((double)950) / ((double)map.getWidth()), ((double)750) / ((double)map.getHeight()) );
				...
				for(int i = 0; i < numPrints; i++){
					if(length >= (radius*2)){
						g2d.drawImage (horizontalRoad, x,
								(int)((map.getHeight() - y)-radius), radius*2, radius*2, null);
					}
					else if(length < (radius*2)){
						g2d.drawImage (horizontalRoad, x,
								(int)((map.getHeight() - y)-radius), length, radius*2, null);
					}
					x = x + (radius*2);
				...
				for(int i = 0; i < numPrints; i++){
					if(length >= (radius*2)){
						g2d.drawImage (verticalRoad,(int)(x-radius),
								(int)(map.getHeight() - y), radius*2, radius*2, null);
					}
					else if(length < (radius*2)){
						g2d.drawImage (verticalRoad,(int)(x-radius),
								(int)(map.getHeight() - y), radius*2, length, null);
					}
					length = length - (radius*2);
					if(length < (radius*2)){
						y = y + length;
		...
		for(Intersection i: intersections)
		{
			 ...
			 */
			g2d.drawImage (intersection,(int)(i.getLocation().getX()-6),(int)((map.getHeight() - i.getLocation().getY())-6), 12, 12, null);
		}
			...
			if(r.isClosed()) {
				...
				for(Closure c: closures){
					g2d.drawImage (closure, (int)(c.getLocation().getX()-radius),
							(int)((map.getHeight() - c.getLocation().getY())-radius),radius*2, radius*2, null);
				}
			}
		}
		...
		if(map.getRobot().getRobotOrientation() == 90){
			g2d.drawImage(robotNORTH,(int)(map.getRobot().getRobotLocation().getX()-7),
					(int)((map.getHeight() - map.getRobot().getRobotLocation().getY())-10), 14, 20, null);
		}
		else if(map.getRobot().getRobotOrientation() == 270){
			g2d.drawImage(robotSOUTH,(int)(map.getRobot().getRobotLocation().getX()-7),
					(int)((map.getHeight() - map.getRobot().getRobotLocation().getY())-10), 14, 20, null);
		}
		else if(map.getRobot().getRobotOrientation() == 0){
			g2d.drawImage(robotEAST,(int)(map.getRobot().getRobotLocation().getX()-10),
					(int)((map.getHeight() - map.getRobot().getRobotLocation().getY())-7), 20, 14, null);
		}
		else if(map.getRobot().getRobotOrientation() == 180){
			g2d.drawImage(robotWEST,(int)(map.getRobot().getRobotLocation().getX()-10),
					(int)((map.getHeight() - map.getRobot().getRobotLocation().getY())-7), 20, 14, null);
		}
